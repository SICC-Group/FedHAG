from FLAlgorithms.users.userpFedRegress import UserFedRegress
from FLAlgorithms.servers.serverbase import Server
from utils.model_utils import read_data, read_user_data, aggregate_user_test_data
import numpy as np

class FedRegress(Server):
    def __init__(self, args, model, seed):
        super().__init__(args, model, seed)

        # Initialize data for all users
        data = read_data(args.dataset)#改成convert_data和convert_test_data
        ...
        for i in range(total_users):
            id, train_data, test_data=read_user_data(i, data, dataset=args.dataset)#label_inform 需要修订
            self.total_train_samples+=len(train_data)
            self.total_test_samples += len(test_data)
            #id, train, test=read_user_data(i, data, dataset=args.dataset)
            user=UserFedRegress(
                args, id, model, train_data, test_data, use_adam=False)#self.unique_labels需要改
    ...
    def train(self, args):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            for user in self.selected_users: # allow selected users to train
                ...
                user.train(
                    glob_iter,
                    personalized=True, lr_decay=True, verbose=True)
            if self.share_model:
                self.aggregate_parameters()
    ...
    #def aggregate_logits(self, selected=True):
        ...
        #for user in users:
            #user_logits += user.RegressionTracker.avg_and_var()
