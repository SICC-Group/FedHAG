...
class FedHAG(Server):
    ...
    def train(self, args):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            for userid,user in enumerate(self.selected_users): # allow selected users to train
                ...
                w_local, loss, indd, last_loss,loss_w = local.train(net=net_local.to(args.device), 
    ...
    def train_generator(self, net_glob,batch_size, global_mean, global_variance, prior, im_out_list, epoches=1, latent_layer_idx=0, verbose=False):
        """
            ...
            for i in range(n_iters):
                ...
                if im_out_list:
                    for im_out in im_out_list:
                        ...
                        # 进行判别器的训练
                        current_real_score = self.discriminator_model(im_out_preprocessed, y)
                        if real_score is None:
                            real_score = current_real_score
                        else:
                            real_score += current_real_score
                    if len(im_out_list) > 0:
                        real_score = real_score / len(im_out_list)
                #### im_out的处理需要修改
        ...
        for i in range(epoches):
            TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS=update_generator_(
                self.n_teacher_iters, global_mean, global_variance, net_glob, TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS)
