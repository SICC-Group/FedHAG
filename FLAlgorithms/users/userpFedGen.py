...
class UserpFedGen(User):
    ...
    def train(self, glob_iter, global_mean, global_variance, personalized=False, early_stop=100, regularization=True, verbose=False):
        ...
        for epoch in range(self.local_epochs):
            ...
            for i in range(self.K):
                ...
                samples =self.get_next_train_batch(self.trainloader,self.iter_trainloader)
                X, y = samples['X'], samples['y']
                y=y.float()
                #self.update_label_counts(samples['labels'], samples['counts'])
                model_result=self.model(X.float())
                user_output_logp = model_result['output']
                RegressionTracker0.update(user_output_logp)
                self.RegressionTracker.update(user_output_logp)
                ...
                if regularization and epoch < early_stop:
                    ...
                    logit_given_gen=self.model(gen_output,start_layer_idx=1)['output']
                    #target_p=F.softmax(logit_given_gen, dim=1).clone().detach()
                    RegressionTracker1.update(logit_given_gen)
                    ...
                    user_output_logp =self.model(gen_output,start_layer_idx=1)['output']
                    teacher_loss =  generative_alpha * torch.mean(
                        self.generative_model.dist_loss(user_output_logp, sampled_y)
                    )
                    # this is to further balance oversampled down-sampled synthetic data
                    gen_ratio = self.gen_batch_size / self.batch_size
                    loss=0.6*predictive_loss + 0.4*reg_loss+gen_ratio * teacher_loss + user_latent_loss
