    ...
    def train(self, net, net_preproc, w_glob_keys, last=False, dataset_test=None, 
        ...
        for iter in range(local_eps):
            ...
            for batch_idx, (data, target) in enumerate(self.dataset):
                ...
                predictions_prior_samples = net(prior_samples)
                loss_ref_dist = self.loss_func(predictions_prior_samples, target)
                num_updates += 1
                ...
                if self.update_prior:
                    ...
                    predictions_probs = net(mu_local)
                    #labels = torch.Tensor([ii for ii in range(self.args.num_classes)]).long()
                    #labels = labels.to(self.args.device)
                    loss = self.loss_func(predictions_probs, target) + lossW
...
def het_test_img_local(net_g, net_preproc, user_data, args,idx=None,indd=None, user_idx=-1, idxs=None):
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        batch_loss = F.mse_loss(prediction, target, reduction='sum').item()
        ...
        if idx==0:
            ...
            y_pred_all = prediction.detach().cpu()
        else:
            target_all = torch.cat((target_all,target.detach().cpu()),dim=0)
            y_pred_all = torch.cat((y_pred_all,prediction.detach().cpu()),dim=0)
